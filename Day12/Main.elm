...
shapesArt shape n pallette ( xgen, ygen, sgen ) =
    Random.map (List.head pallette |> Maybe.withDefault Color.black |> withBackground)
        (Random.list n
            (Random.map4
                (smallShape shape)
                xgen
...
randomPalette =
    Random.Extra.frequency
        ...
        , ( 1, Random.list 3 (Random.Color.greyscale) )
          -- , ( 1, Random.list 5 (Random.Color.red) )
          -- , ( 1, Random.list 5 (Random.Color.blue) )
          -- ,
        , ( 3, Random.Extra.flatMap monochromePalette (Random.float 0 360) )
        ]
