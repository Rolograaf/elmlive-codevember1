...
renderBlade index blade =
    Svg.polygon
        [ Svg.Attributes.points <| "-1,0 0," ++ toString (-blade.size) ++ " 1,0"
        , Svg.Attributes.fill (Color.Convert.colorToHex blade.color)
        , Svg.Attributes.transform <|
            String.join " "
                [ "translate(" ++ toString (index - 50 // 2) ++ ",0)"
...
view model =
    Svg.svg
        ...
        , Html.Attributes.style [ ( "max-height", "100%vh" ) ]
        ]
        [ Svg.rect
        ...
        , List.indexedMap renderBlade model.blades
            |> Svg.g
                [ Svg.Attributes.transform "translate(0,35)"
                ]
        ]
...
update msg model =
    case msg of
        ...
        GrowBlades growths ->
            let
                growBlade ( index, color ) =
                    grow (toFloat model.mousePosition.x / toFloat model.windowSize.width)
                        color
                        index
            in
                ( { model | blades = List.foldl growBlade model.blades growths }
                    |> checkDone
                , Cmd.none
                )
...
main =
    Html.program
        { init = ( initialModel, Window.size |> Task.perform WindowSize )
        , subscriptions =
            \model ->
                Sub.batch
                    [ if model.done then
